	...
	function agent_save($option,$save)
    {
		...
		if($username != '' && $password != '' && $existing_user == 0)
		{
			...
			if($count > 0)
			{
				if($isNew == 1)
				{
					$mainframe->enqueueMessage(JText::_('OS_USER_EXISTING'), 'error');
		...
		else
		{
			...
			if($user_id > 0)
			{
				...
				if($count > 0)
				{
					if($isNew == 1)
					{
						$mainframe->enqueueMessage(JText::_('OS_JOOMLA_USER_HAS_BEEN_ASSIGNED_TO_ANOTHER_AGENT'), 'error');
						$mainframe->redirect("index.php?option=com_osproperty&task=agent_new");
					}
					else
					{
				...
				if($count > 0)
				{
					if($isNew == 1)
					{
						$mainframe->enqueueMessage(JText::_('OS_THIS_JOOMLA_USER_ALREADY_IS_COMPANY_ADMIN'), 'error');
						$mainframe->redirect("index.php?option=com_osproperty&task=agent_new");
					}
					else
					{
		...
		if (!empty($_FILES['file_photo']['name']) && $_FILES['file_photo']['error'] == 0 && $_FILES['file_photo']['size'] > 0 )
		{
			if(!HelperOspropertyCommon::checkIsPhotoFileUploaded('file_photo'))
			{
				...
				if($isNew == 1){
					$mainframe->enqueueMessage(JText::_('OS_PHOTO_INVALID'), 'error');
					$mainframe->redirect("index.php?option=com_osproperty&task=agent_new");
				}else{
					$mainframe->enqueueMessage(JText::_('OS_PHOTO_INVALID'), 'error');
					$mainframe->redirect("index.php?option=com_osproperty&task=agent_edit&cid[]=".$id);
