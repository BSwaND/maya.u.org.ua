...
class HTML_OspropertyCsvform{
	...
	function updateOtherInformationForm($option,$properties,$row,$lists){
		...
		?>
		<script type="text/javascript" src="<?php echo JURI::root()?>media/com_osproperty/assets/js/ajax.js"></script>
	...
	function importCsvForm($option,$row,$lists){
		...
		<div style="width:100%;">
			<fieldset>
				...
				if(($market_status != "") && ($configClass['active_market_status'])){
					?>
					<span class="badge badge-success">7</span>
					<?php
					echo JText::_('OS_MARKET_STATUS_FIELD');
					...
					<ul>
						...
						if(in_array('1',$market_status_array)){
							?>
							<li>
								<?php echo JText::_('OS_SOLD');?> - <strong>1</strong>
							</li>
							<?php
						}
						?>
						...
						if(in_array('2',$market_status_array)){
							?>
							<li>
								<?php echo JText::_('OS_CURRENT');?> - <strong>2</strong>
							</li>
							<?php
						}
						?>
						...
						if(in_array('3',$market_status_array)){
							?>
							<li>
								<?php echo JText::_('OS_RENTED');?> - <strong>3</strong>
							</li>
							<?php
						}
						?>
					...
					<tr class="<?php echo "row$k"; ?>">
						...
						<td style="text-align:center;">
							<a href="index.php?option=com_osproperty&task=form_downloadcsv&cid[]=<?php echo $row->id?>&no_html=1" title="<?php echo JText::_('OS_DOWNLOAD_CSV_FORM')?>">
								<img src="<?php echo JURI::root()?>media/com_osproperty/assets/images/csv16.png" border="0" >
							</a>
						</td>
						<td style="text-align:center;">
						...
						<td style="text-align:center;">
							<a href="index.php?option=com_osproperty&task=form_importcsv&cid[]=<?php echo $row->id?>" title="<?php echo JText::_('OS_IMPORT_CSV')?>">
								<img src="<?php echo JURI::root()?>media/com_osproperty/assets/images/import16.png" border="0">
							</a>
						</td>
						<td>
	...
	function editHTML($option,$row,$lists){
		...
		<table cellpadding="0" cellspacing="0" width="100%" class="admintable">
			...
			<tr>
				...
				<td>
					<div style="border:1px solid #CCC;background-color:#E4EFFC;padding:10px;">
						...
						<?php echo JText::_('OS_IMPORT_PICTURE_FROM');?>:
						<?php
						if($row->image_type == 0){
							$tchecked1 = "checked";
							$tchecked2 = "";
						...
						?>
						<div style="padding-left:35px;">
							<input type="radio" name="image_type" id="image_type" value="0" <?php echo $tchecked1;?> /> <?php echo JText::_('OS_FROM_COMPRESS_FILE')?>
							<BR />
							<input type="radio" name="image_type" id="image_type" value="1" <?php echo $tchecked2;?>/> <?php echo JText::_('OS_FROM_OTHER_HOSTING')?>
						</div>
						<BR/>
						<span class="badge badge-warning">11</span> &nbsp;
						<?php echo JText::_('OS_UPDATE_PROPERTIES_WHEN');?>:
						<?php
						...
						?>
						<div style="padding-left:35px;">
							<input type="radio" name="update_type" id="update_type" value="0" <?php echo $tchecked1;?> /> <?php echo JText::_('OS_HAVING_SAME_REF_AND_TITLE')?>
							<BR />
							<input type="radio" name="update_type" id="update_type" value="1" <?php echo $tchecked2;?>/> <?php echo JText::_('OS_HAVING_SAME_REF')?>
						</div>
					</div>
				</td>
			</tr>
			<tr>
				<td class="key" valign="top">
					<?php
					echo JText::_('OS_ACTIVE_CRON_IMPORT');
					?>
				</td>
				<td>
					<?php echo $lists['active_cron_import'];?>
					<BR />
					<?php echo JText::_('OS_ACTIVE_CRON_IMPORT_EXPLAIN');?>
					<BR />
					<div class="csv_import">
						<strong><?php echo JText::_('OS_CSV_PATH'); ?></strong>
						<?php echo JPATH_ROOT.'/';?><input type="text" id="csv_file" class="input-xxlarge" name="csv_file" value="<?php echo $row->csv_file; ?>" />
					</div>
				...
				<td style="padding:10px;background-color:#FCEAF3;border:1px solid #FED2E8;">
					<div style="width:100%">
						<div style="float:left;margin-right:10px;">
							<img src="<?php echo JURI::root()?>media/com_osproperty/assets/images/notice.png">
						</div>
						<?php
