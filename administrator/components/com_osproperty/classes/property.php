...
class OspropertyProperties
{
	...
	function properties_list($option){
		...
		$lists['cities'] = HelperOspropertyCommon::makeCityList($country_id,$state_id, $city_id, 'city_id','onChange="javascript:document.adminForm.submit();"',JText::_('OS_ALL_CITIES'),'class="chosen input-medium"');
		//number bed rooms
	...
	function doUploadPhotoPackages($option){
		...
		$mainframe->enqueueMessage(JText::_('OS_COMPLETE_INSTALL_SAMPLE_DATA'));
...
//		$query = 'SELECT *' .
	...
	function properties_edit($option,$id){
		...
		$lists['price_call'] = JHTML::_('select.genericlist',$optionArr,'price_call','class="input-mini" onChange="javascript:showPriceFields()"','value','text',(int)$row->price_call);
		...
		$lists['country'] = HelperOspropertyCommon::makeCountryList($row->country,'country','onChange="javascript:loadStateBackend(this.value,\''.$row->state.'\',\''.$row->city.'\')"','','class="input-large"');
		...
		if(OSPHelper::userOneState()){
			...
			$lists['states'] = HelperOspropertyCommon::makeStateList($row->country,$row->state,'state','onChange="javascript:loadCityBackend(this.value,\''.$row->city.'\')"',JText::_('OS_SELECT_STATE'),'class="input-large"');
		}
		...
		if($id > 0){
			...
			$lists['city'] = HelperOspropertyCommon::loadCity($option,$row->state,$row->city,'input-large');
		}else{
			$default_state = 0;
			if(OSPHelper::userOneState()){
				$default_state = OSPHelper::returnDefaultState();
			}
			$lists['city'] = HelperOspropertyCommon::loadCity($option,$default_state,0,'input-large');
		}
		
		//number bed rooms
		$lists['nbed'] 		= OSPHelper::dropdropBed('bed_room',$row->bed_room,'input-small','','OS_BED');
		//number bath rooms
		$lists['nbath'] 	= OSPHelper::dropdropBath('bath_room',$row->bath_room,'input-small','','OS_BATH');
		//number rooms
		$lists['nrooms'] 	= OSPHelper::dropdropRoom('rooms',$row->rooms,'input-small','','OS_ROOMS');
        //number floors
	...
	function getUserInput($agent_id){
		...
		else
		{
            ...
            JFormHelper::addFieldPath(JPATH_ADMINISTRATOR . '/components/com_osproperty/fields');

            if (version_compare(JVERSION, '4.0.0-dev', 'ge'))
            {
                JFormHelper::addFieldPrefix('Joomla\Component\Osproperty\Administrator\Field');

            }
            $field = JFormHelper::loadFieldType('Modal_Agent');
...
//			echo $address_search;
            ...
            {
                include_once(JPATH_SITE.DS."components".DS."com_osproperty".DS."helpers".DS."openstreetmap.lib.php");
		...
		$img_ids = JFactory::getApplication()->input->get('img_id',array(),'array');
    	$img_captions = JFactory::getApplication()->input->get('img_caption',array(),'array');
    	$img_images = JFactory::getApplication()->input->get('img_image',array(),'array');
		...
		if ($isNew == 1)
		{
			JFactory::getApplication()->triggerEvent('onAfterStoreProperty', array($row));
		}
	...
	function generatePhoto($option){
		...
		$db = JFactory::getDbo();
		$document = JFactory::getDocument();
		$document->addScript(JURI::root()."media/com_osproperty/assets/js/yui/script/yuiloader-min.js");
		$document->addScript(JURI::root()."media/com_osproperty/assets/js/yui/script/yahoo-dom-event.js");
		$document->addScript(JURI::root()."media/com_osproperty/assets/js/yui/script/element-min.js");
		$document->addScript(JURI::root()."media/com_osproperty/assets/js/yui/script/dragdrop-min.js");
		$document->addScript(JURI::root()."media/com_osproperty/assets/js/yui/script/imagecropper-min.js");
		$document->addScript(JURI::root()."media/com_osproperty/assets/js/yui/script/resize-min.js");
		$document->addScript(JURI::root()."media/com_osproperty/assets/js/yui/script/event-min.js");
		$document->addScript(JURI::root()."media/com_osproperty/assets/js/yui/script/dom-min.js");
    ...
    function doOptimizeSefUrls($option){
    	...
    	$db->execute();
    	$mainframe->enqueueMessage(JText::_('OS_SEF_URLS_OPTIMIZATION_HAS_BEEN_COMPLETED'));
    ...
    function doSyncdatabase($option){
    	...
    	$mainframe->enqueueMessage(JText::_('OS_DATABASE_SYNCHRONOUS_COMPLETED'));
    	$mainframe->redirect("index.php?option=com_osproperty");
    }
    		...
    		for($i=0;$i<count($properties);$i++){
    			...
    			if(count($photos) > 0){
    				for($j = 0;$j<count($photos);$j++){
						...
						if(JFile::exists($backupurl)){
							JFile::copy($backupurl,$originalurl);
						}
    ...
    function fixdatabase(){
        ...
        $mainframe->enqueueMessage(JText::_('OS_DATABASE_SCHEMA_HAS_BEEN_FIXED'));
        $mainframe->redirect("index.php?option=com_osproperty");
    }
	...
	**/
    ...
    function doremoveOrphan(){
        ...
        $mainframe->enqueueMessage(JText::_('OS_ORPHAN_PROPERTIES_HAVE_BEEN_REMOVED'));
        JFactory::getApplication()->redirect('index.php?option=com_osproperty');
    }
