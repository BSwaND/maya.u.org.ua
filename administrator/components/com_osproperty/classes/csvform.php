...
class OspropertyCsvform{
	...
	public static function importCell($row,$cell,$import_utf){
		...
		if($city != ""){
			...
			if(intval($cityid) == 0){//can't find the type
				if($row->fcity == 0){
					JTable::addIncludePath(JPATH_ADMINISTRATOR . '/components/com_osproperty/tables');
		...
		if(!$property->store()){
			...
			$error .= "<strong>".JText::_('OS_ADDRESS')."</strong>: ".$property->address;
			$isInsert = 1;
			if(count($catIds) > 0){
	...
	public static function editForm($option,$id){
		...
		$optionArr[] = JHTML::_('select.option','0',JText::_('OS_NO'));
		$optionArr[] = JHTML::_('select.option','1',JText::_('OS_YES'));
		$lists['active_cron_import']= JHtml::_('select.genericlist',$optionArr,'active_cron_import','class=input-small','value','text',$row->active_cron_import);
		...
		$labels[] = JText::_('OS_ORIGINAL_PRICE');
		$labels[] = JText::_('OS_PRICE_TEXT');
		...
		$labels[] = JText::_('OS_MARKET_STATUS');
		$labels[] = JText::_('OS_META_DESCRIPTION');
		$labels[] = JText::_('OS_ISFEATURED');
