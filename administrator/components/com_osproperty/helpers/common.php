...
class HelperOspropertyCommon{
	...
	**/
	 ...
	 * @return unknown
	 */
	function loadCity($option,$state_id,$city_id,$class="input-medium"){
		global $mainframe;
		$db = JFactory::getDBO();
		$cityArr = array();
		...
		return JHTML::_('select.genericlist',$cityArr,'city','class="chosen '.$class.'"'.$disabled,'value','text',$city_id);
	}
	...
	public static function makeCountryList($req_country_id,$name,$onChange,$firstOption,$style){
		...
		if($configClass['show_country_id'] != ""){
			if(HelperOspropertyCommon::checkCountry()){
				...
				return  JHTML::_('select.genericlist',$countryArr,$name,'class="chosen" '.$onChange.' '.$style,'value','text',$req_country_id);
	...
	public static function renderSubmenu($vName = 'dashboard')
	{
		...
		for ($i = 0; $n = count($menus), $i < $n; $i++)
		{
			...
			else
			{
				...
				{
					$subMenu = $subMenus[$j];
					$lName = JFactory::getApplication()->input->getString('layout','');
				...
				$html .= '<li' . $class . '>';
				$html .= '<a id="drop_' . $menu->id . '" href="#" data-toggle="dropdown" role="button" class="dropdown-toggle nav-link dropdown-toggle">' ;
				if($menu->menu_icon != ""){
					$html .= '<i class="'.$menu->menu_icon.'"></i>';
				}
