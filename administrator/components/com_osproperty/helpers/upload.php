...
class JoomUpload extends JObject
{
  ...
  protected function uploadAJAX($pid)
  {
    ...
    $image               = JFactory::getApplication()->input->files->get('qqfile');
    $qqtotalfilesize     = JFactory::getApplication()->input->getInt('qqtotalfilesize', -1);
    $totalParts          = JFactory::getApplication()->input->getInt('qqtotalparts', 1);
    $screenshot          = $image['tmp_name'];
    $origfilename        = JFactory::getApplication()->input->getString('qqfilename', '');
    ...
    if($totalParts > 1)
    {
      $partIndex    = JFactory::getApplication()->input->getInt('qqpartindex',1);
      $uuid         = JFactory::getApplication()->input->getInt('qquuid',1);
