...
class HelperOspropertyFields{
	...
	function setFieldValue($list_detail){
		...
		if($list_detail->field_type == 1){
			...
			switch ($field->field_type){
				case "date":
				case "text":
					JFactory::getApplication()->input->set('type_'.$field->field_name,$list_detail->search_type);
					JFactory::getApplication()->input->set($field->field_name,$list_detail->search_param);
				...
				case "multipleselect":
					...
					JFactory::getApplication()->input->set($field->field_name,$search_param);
					break;
			}
		}
	}
	...
	function showField_Textarea($field,$pid){
		...
		if(($obj->value == "") and ($field->default_value!="")){
	...
	function saveField_Checkbox($field,$pid){
		...
		$valueArr = \Joomla\Utilities\ArrayHelper::toInteger($valueArr);
		$db->setQuery("DELETE FROM #__osrs_property_field_opt_value WHERE pid = '$pid' AND fid = '$field->id'");
		$db->execute();
		if(count($valueArr) > 0){
