		...
		<table class="ptable table table-striped tablelistproperties" id="propertyList">
			...
			if(count($rows) > 0){
				...
				for($i=0;$i<count($rows);$i++){
					$row            = $rows[$i];
					$link           = JRoute::_('index.php?option=com_osproperty&task=property_stas&id='.$row->id);
					...
					<tr class="row<?php echo $k;?>">
						...
						<td class="has-context" data-label="<?php echo JText::_('OS_LOCATION').' / '.JText::_('OS_TITLE').' / '.JText::_('Ref #').' / '.JText::_('OS_PRICE');?>">
							<div class="pull-left">
								<a data-toggle="tooltip" style="display:inline-block !important;" class="hasTip" title="<img src='<?php echo $row->photo;?>'/>"><i class="osicon-camera"></i></a> | 
						...
						<td class="small center" data-label="<?php echo JText::_('OS_FEATURED')?>">
							...
							if($supervisor == 0){
								...
								if($row->isFeatured == 1){
								?>
									<a class="btn btn-micro active btn-success" href="javascript:changeStatus(<?php echo $row->id?>,'isFeatured',0);" title="<?php echo JText::_('OS_CLICK_HERE_TO_CHANGE_FEATURED_STATUS');?>">
										<i class="osicon-star"></i>
									</a>
								<?php
								}else{
									?>
									<a class="btn btn-micro active btn-danger" title="<?php echo JText::_('OS_CLICK_HERE_TO_CHANGE_FEATURED_STATUS')?>" href="javascript:changeStatus(<?php echo $row->id?>,'isFeatured',1);">
										<i class="osicon-star osicon-white"></i>
									</a>
									<?php
								}
						...
						<td class="small center" data-label="<?php echo JText::_('OS_APPROVED')?>">
							...
							if($supervisor == 0){
								if($row->approved == 1){
									...
									if($configClass['general_use_expiration_management'] == 1){
										?>
										<a class="btn btn-micro active btn-warning" title="<?php echo JText::_('OS_EXTEND_LIVE_TIME')?>" href="<?php echo $extend_link;?>">
								...
								if($row->approved == 1){
								?>
									<a class="btn btn-micro active btn-success" href="javascript:changeStatus(<?php echo $row->id?>,'approved',0);" title="<?php echo JText::_('OS_CLICK_HERE_TO_CHANGE_APPROVAL_STATUS');?>">
										<i class="osicon-star"></i>
									</a>
								<?php
								}else{
									?>
									<a class="btn btn-micro active btn-danger" title="<?php echo JText::_('OS_CLICK_HERE_TO_CHANGE_APPROVAL_STATUS')?>" href="javascript:changeStatus(<?php echo $row->id?>,'approved',1);">
										<i class="osicon-star osicon-white"></i>
									</a>
									<?php
								}
						...
						<td class="small center" data-label="<?php echo JText::_('OS_PUBLISH')?>">
							...
							if($supervisor == 0){
								...
								if($row->published == 1){
									?>
									<a class="btn btn-micro active btn-success" href="javascript:changeStatus(<?php echo $row->id?>,'published',0);" title="<?php echo JText::_('OS_CLICK_HERE_TO_CHANGE_PUBLISH_STATUS');?>">
										<i class="osicon-star"></i>
									</a>
									<?php
								}else{
									?>
									<a class="btn btn-micro active btn-danger" title="<?php echo JText::_('OS_CLICK_HERE_TO_CHANGE_PUBLISH_STATUS')?>" href="javascript:changeStatus(<?php echo $row->id?>,'published',1);">
										<i class="osicon-star osicon-white"></i>
									</a>
									<?php
								}
						...
						<td class="small center" data-label="<?php echo JText::_('OS_PRINT')?>">
							...
							<a href="javascript:void(0);" onclick="javascript:<?php echo $open_print?>;" class="<?php echo $bootstrapHelper->getClassMapping('btn');?>" title="<?php echo JText::_('OS_PRINT')?>">
							...
							if(file_exists(JPATH_ROOT.DS."components".DS."com_oscalendar".DS."oscalendar.php")){
								if($configClass['integrate_oscalendar'] == 1){
									...
									<td align="center" class="data_td center" style="background-color:<?php echo $bgcolor?>;" data-label="<?php echo JText::_('OS_MANAGE_ROOMS')?>">
										<a href="<?php echo JRoute::_("index.php?option=com_oscalendar&task=room_manage&pid=".$row->id."&Itemid=".$jinput->getInt('Itemid',0));?>" target="_blank" title="<?php echo JText::_('OS_MANAGE_ROOMS')?>" class="btn" />
