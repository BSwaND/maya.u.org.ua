...
class OSPRoute
{
		...
		if(count($needs) > 0){
			...
			if($needs[0] == "property_details"){
				...
				if($pid > 0){
                    ...
                    $category_id    = OSPHelper::getCategoryIdsOfProperty($pid);
	...
	static function compareMenuItem($itemid,$catIds,$type_id,$country_id,$company_id,$state_id){
		$db = JFactory::getDbo();
		...
		if($count > 0){
			...
			if((self::compareArray($catIds, $find_category_id)) && ($type_id == $find_pro_type) && ($country_id == $find_country) && ($company_id == $find_company_id) && ($state_id == $find_state_id)){
				return true;
			}else{
				return false;
			}
