...
class OspropertyAgent
{
	...
	static function agentLayout($option)
    {
		...
		$query                  = "Select count(a.id) from #__osrs_agents as a inner join #__users as b on b.id = a.user_id ";
		...
		$query                  = "Select a.* from #__osrs_agents as a inner join #__users as b on b.id = a.user_id " ;
        if ($show_agent_with_properties == 1)
        {
            $query              .= " inner join #__osrs_properties as c on a.id = c.agent_id";
        }
		...
		$lists['country'] = HelperOspropertyCommon::makeCountryList($country_id,'country_id','onchange="change_country_company(this.value,'.$state_id.','.$city.')"',JText::_('OS_ANY'),'','input-medium');
	...
	static function editProfile($option){
		...
		$lists['country'] = HelperOspropertyCommon::makeCountryList($agent->country,'country','onChange="javascript:loadState(this.value,\''.$agent->state.'\',\''.$agent->city.'\')"',JText::_('OS_SELECT_COUNTRY'),'','input-large');

		if(OSPHelper::userOneState()){
			$lists['state'] = "<input type='hidden' name='state' id='state' value='".$agent->state."'/>";
		}else{
			$lists['state'] = HelperOspropertyCommon::makeStateListAddProperty($agent->country,$agent->state,'state','onChange="javascript:loadCity(this.value,\''.$agent->city.'\')"',JText::_('OS_SELECT_STATE'),'class="input-large"');
		}
		if(intval($agent->state) == 0){
			$agent->state = OSPHelper::returnDefaultState();
		}
		$lists['city']  = HelperOspropertyCommon::loadCityAddProperty($option,$agent->state,$agent->city,"input-large");
		...
		$lists['orderby'] = JHTML::_('select.genericlist',$orderbyArr,'orderby','class="input-medium" onchange="javascript:this.form.submit();"','value','text',$orderby);
		...
		$lists['sortby'] = JHTML::_('select.genericlist',$sortbyArr,'sortby','class="input-medium" onchange="javascript:this.form.submit();"','value','text',$sortby);
        ...
        $lists['optin'] = JHTML::_('select.genericlist',$optArr,'optin','class="input-medium"','value','text',(int)$agent->optin);
	...
	function editProperties($option,$id){
		global $mainframe,$jinput;
		OSPHelper::redirect("index.php?option=com_osproperty&task=property_edit&id=$id&Itemid=".$jinput->getInt('Itemid',0));
	}
	...
	static function agentRegister($option){
		...
		$lists['country'] = HelperOspropertyCommon::makeCountryList($country,'country','onchange="change_country_company(this.value,0,0)"',JText::_('OS_SELECT_COUNTRY'),'','input-large');
		
		if(OSPHelper::userOneState()){
			$lists['state'] = "<input type='hidden' name='state' id='state' value='".OSPHelper::returnDefaultState()."'/>".OSPHelper::returnDefaultStateName();
		}else{
			$lists['state'] = HelperOspropertyCommon::makeStateListAddProperty('',$state,'state','onchange="loadCity(this.value,0)"',JText::_('OS_SELECT_STATE'),'class="input-large"');
		}
		if(OSPHelper::userOneState()){
			$default_state = OSPHelper::returnDefaultState();
		}else{
		...
		$lists['city'] = HelperOspropertyCommon::loadCity($option,$default_state,$city,'input-large');
