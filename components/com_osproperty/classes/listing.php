...
class OspropertyListing{
	...
	static function advSearch($option){
        ...
        $sortby			= OSPHelper::getStringRequest('sortby',$configClass['adv_sortby']);
        $orderby		= OSPHelper::getStringRequest('orderby',$configClass['adv_orderby']);
        $orderbyArray   = array('a.pro_name','a.id','a.modified','a.price','a.isFeatured','a.square_feet');
		...
		$lists['nbath'] = JHTML::_('select.genericlist',$bathArr,'nbath',' class="input-small"','value','text',$nbath);
		...
		$lists['nbed'] = JHTML::_('select.genericlist',$bedArr,'nbed','class="input-small"','value','text',$nbed);
		...
		$lists['nroom'] = JHTML::_('select.genericlist',$roomArr,'nroom','class="input-small"','value','text',$nroom);
		...
		$lists['nfloor'] = JHTML::_('select.genericlist',$floorArr,'nfloors','class="input-small"','value','text',$nfloors);
		
		
		//country
		$lists['country'] = HelperOspropertyCommon::makeCountryList($country_id,'country_id','onchange="change_country_company(this.value)"',JText::_('OS_ALL_COUNTRIES'),'','input-large');
		...
		$lists['radius'] = JHtml::_('select.genericlist',$radiusArr,'radius_search','class="input-medium"', 'value', 'text',$radius_search);
	...
	static function listProperties($option,$company_id,$category_ids,$agent_id,$property_type,$keyword,$nbed,$nbath,$isfeatured,$isSold,$nrooms,$orderby,$ordertype,$limitstart,$limit,$favorites,$price,$filterParams,$city_id,$state_id,$country_id,$max_properties,$show_filterform,$agenttype=-1,$tag_id = 0,$min_price=-1,$max_price=-1){
		...
		$protypes = $db->loadObjectList();
		$typeArr   = array_merge($typeArr,$protypes);
		$lists['type'] = JHTML::_('select.genericlist',$typeArr,'property_type','class="input-medium chosen"','value','text',$property_type);
		//categories
		...
		$lists['nrooms'] = JHTML::_('select.genericlist',$roomsArr,'nrooms','class="input-small"','value','text',$nrooms);
	...
	static function details($option){
		...
		if(($property->published == 0) or ($property->approved == 0)){
			...
			if($redirect_link != ""){
				OSPHelper::redirect($redirect_link,JText::_("OS_PROPERTY_IS_NOT_AVAILABLE"));
			}else{
				JError::raiseError( 404, JText::_('OS_PROPERTY_IS_NOT_AVAILABLE') );
			}
		}
		...
		else
		{
            ...
            foreach ($optionArr as $amen_cat)
			{
                ...
                if(count($amens) > 0){
                    ...
                    ?>
					<h5><strong><i class="edicon <?php echo $amenityCategoryIcons[$icon];?>"></i>&nbsp;<?php echo $amen_cat?></strong></h5>
	...
	static function edit($option,$id, $type = 0){
		...
		if($user->id == 0){
			OSPHelper::redirect(JUri::root(),JText::_('OS_LOGIN_WARNING'));
		}
		...
		$lists['show_address'] = JHTML::_('select.genericlist',$optionArr1,'show_address','class="input-large"','value','text',$row->show_address);
		$lists['price_call'] = JHTML::_('select.genericlist',$optionArr,'price_call','class="input-large chosen" onChange="javascript:showPriceFields()"','value','text',$row->price_call);
		$lists['property_sold'] = JHTML::_('select.genericlist',$optionArr,'isSold','class="input-large chosen"','value','text',$row->isSold);
		...
		$lists['country'] = HelperOspropertyCommon::makeCountryList($row->country,'country','onChange="javascript:loadState(this.value,\''.$row->state.'\',\''.$row->city.'\')"',JText::_('OS_SELECT_COUNTRY'),'','input-large');
		
		if(OSPHelper::userOneState()){
			$lists['state'] = "<input type='hidden' name='state' id='state' value='".OSPHelper::returnDefaultState()."'/>";
		}else{
			$lists['state'] = HelperOspropertyCommon::makeStateListAddProperty($row->country,$row->state,'state','onChange="javascript:loadCity(this.value,\''.$row->city.'\')"',JText::_('OS_SELECT_STATE'),'class="input-large"');
		}
		...
		if($id > 0){
			...
			$lists['city'] = HelperOspropertyCommon::loadCityAddProperty($option,$row->state,$row->city,"input-large");
		}else{
			$default_state = 0;
			if(OSPHelper::userOneState()){
				$default_state = OSPHelper::returnDefaultState();
			...
			$lists['city'] = HelperOspropertyCommon::loadCityAddProperty($option,$default_state,$row->city,"input-large");
		}
		...
		$lists['time'] = JHTML::_('select.genericlist',$timeArr,'rent_time','class="input-large chosen"','value','text',$row->rent_time);
		//html
		...
		if($configClass['integrate_membership'] == 1){ //allow to integrate with membership
			if(intval($id) == 0){
				...
				if(!HelperOspropertyCommon::checkMembershipIsAvailable()){
					...
					OSPHelper::redirect($redirect_link,JText::_('OS_YOU_NEED_TO_PURCHASE_SUBSCRIPTION_IF_YOU_WANT_TO_ADD_PROPERTY'));
				}else{
		...
		*/
		...
		if($configClass[''] == 0){
			$document->addScript(JURI::root().'media/com_osproperty/assets/js/djuploader.js','text/javascript',true);
			$settings = array();
	...
	static function save($option,$save)
    {
		...
		if(is_uploaded_file($_FILES['panorama']['tmp_name']))
		{
			if(is_uploaded_file($_FILES['panorama']['tmp_name']))
			{
				if($document_file != "")
				{ // remove old file
					@unlink(JPATH_ROOT.DS."images".DS."osproperty".DS."properties".DS."panorama".DS.$document_file);
				}
				$panorama = time()."_".$_FILES['panorama']['name'];
				move_uploaded_file($_FILES['panorama']['tmp_name'],JPATH_ROOT.DS."images".DS."osproperty".DS."properties".DS."panorama".DS.$panorama);
				$row->panorama = $panorama;
			}
		}
		if($remove_panorama == 1)
		{
			if($panorama_file != "")
			{ // remove old file
				@unlink(JPATH_ROOT.DS."images".DS."osproperty".DS."properties".DS."panorama".DS.$document_file);
		...
		if(($lat_add == "") || ($long_add == ""))
		{
			...
			$sname = OSPHelper::loadSateName($state);//$db->loadResult();
					...
					if(is_array($new_img_name)){
						...
						if (JFile::exists($new_img_name_u)){
							if(getimagesize($new_img_name_u)){
								...
								$thumb_image_link = JPATH_ROOT.DS."images".DS."osproperty".DS."properties".DS.$id.DS."thumb".DS.$new_img_n;
								...
								$medium_image_link = JPATH_ROOT.DS."images".DS."osproperty".DS."properties".DS.$id.DS."medium".DS.$new_img_n;
		...
		if ($isNew == 1)
		{
			$dispatcher->trigger('onAfterStoreProperty', array($row));
		}
		...
		if (JFactory::getUser()->authorise('frontendmanage', 'com_osproperty')) {
            ...
            //check if active payment is turned on.
			if(($configClass['active_payment'] == 1) and ($passcheck == 0) and ($configClass['integrate_membership'] == 0)){
				...
				$order->payment_method = OSPHelper::getStringRequest('payment_method','');
				...
				//process payment
				if(floatVal($total) > 0){
					OSPHelper::redirect(JURI::root().'index.php?option=com_osproperty&direction=add_new&task=payment_process&order_id='.$order_id.'&Itemid='.$jinput->getInt('Itemid',0));
				}else{
					...
					$db->execute();
					OSPHelper::redirect(JURI::root()."index.php?option=com_osproperty&task=property_thankyou&id=$id&Itemid=".$jinput->getInt('Itemid',0));
				}
					...
					if($isNew == 0){
						if($save == 1){
							if(HelperOspropertyCommon::isAgent()){
								...
								$itemid = OSPRoute::getItemid($needs);
								OSPHelper::redirect(JRoute::_("index.php?option=com_osproperty&task=company_properties&Itemid=".$itemid),JText::_('OS_ITEM_HAS_BEEN_SAVE'));
							}
						}else{
							OSPHelper::redirect(JRoute::_("index.php?option=com_osproperty&task=property_edit&id=".$id),JText::_('OS_ITEM_HAS_BEEN_SAVE'));
						}
					}else{
						OSPHelper::redirect(JRoute::_("index.php?option=com_osproperty&task=property_thankyou&id=$id&Itemid=".$jinput->getInt('Itemid',0)));
					}
				}elseif($configClass['custom_thumbnail_photo'] == 1){
					OSPHelper::redirect(JURI::root()."index.php?option=com_osproperty&task=property_generatephoto&pid=$id&save=$save&isNew=$isNew&photoIds=".implode(",",$photoIds));
				}else{
					OSPHelper::generateWaterMark($id);
					$msg = JText::_('OS_ITEM_SAVED');
					if($isNew == 0){
	...
	function membershipprocess($cid){
        global $mainframe,$configClass,$jinput;
        $db                     = JFactory::getDBO();
        $type                   = $jinput->getInt('type',0);
        if( !HelperOspropertyCommon::isAgent() && !HelperOspropertyCommon::isCompanyAdmin()){
	...
	static function showPanorama($id){
		...
		$db = JFactory::getDbo();
		$query = $db->getQuery(true);
		$query->select('panorama')->from('#__osrs_properties')->where('id="'.$id.'"');
		$db->setQuery($query);
		$panorama = $db->loadResult();
