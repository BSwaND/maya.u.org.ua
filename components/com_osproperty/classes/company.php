...
class OspropertyCompany{
	...
	static function editCompany($option){
		...
		$lists['country'] = HelperOspropertyCommon::makeCountryList($row->country,'country','onchange="change_country_company(this.value,'.$row->state.','.$row->city.')"',JText::_('OS_SELECT_COUNTRY'),'','input-large');

		if(OSPHelper::userOneState()){
			$lists['state'] = "<input type='hidden' name='state' id='state' value='".$row->state."'/>";
		}else{
			$lists['state'] = HelperOspropertyCommon::makeStateListAddProperty($row->country,$row->state,'state','onchange="change_state_company(this.value,'.intval($row->city).')"',JText::_('OS_SELECT_STATE'),'class="input-large"');
		}
        ...
        $lists['optin'] = JHTML::_('select.genericlist',$optArr,'optin','class="input-medium"','value','text',(int)$row->optin);
	...
	static function modifyAgent($option,$id){
		...
		$lists['city'] = HelperOspropertyCommon::loadCityAddProperty($option,$default_state,$agent->city,"input-large");
	...
	static function companyRegister($option){
		...
		$lists['country'] = HelperOspropertyCommon::makeCountryList($country,'country','onchange="change_country_company(this.value,\''.$state.'\',\''.$city.'\')"',JText::_('OS_SELECT_COUNTRY'),'');
		
		if(OSPHelper::userOneState()){
			$lists['state'] = "<input type='hidden' name='state' id='state' value='".OSPHelper::returnDefaultState()."'/>";
		}else{
			$lists['state'] = HelperOspropertyCommon::makeStateListAddProperty('',$state,'state','onchange="loadCity(this.value,\''.$city.'\')"',JText::_('OS_SELECT_STATE'),'class="input-large"');
		}
		...
		$lists['city'] = HelperOspropertyCommon::loadCityAddProperty($option,$default_state,$city,"input-large");
	...
	static function saveNewCompany($option){
		...
		if($configClass['show_company_captcha'] == 2){
			$post = $jinput->post->getArray();
		...
		if($configClass['show_company_captcha'] == 1){
			$comment_security_code = $jinput->getString('security_code','');
			$captcha_str = $_POST['captcha_str'];
			if($comment_security_code == ''){
				OSPHelper::redirect(JRoute::_('index.php?option=com_osproperty&task=company_register&Itemid='.$jinput->getInt('Itemid',0)),JText::_('OS_SECURITY_CODE_IS_WRONG'));
			}
