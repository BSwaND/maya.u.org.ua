...
class HTML_OspropertyListing{
	...
	static function advSearchForm($option,$groups,$lists,$rows,$pageNav,$param,$adv_type,$dosearch){
		...
		<div class="<?php echo $bootstrapHelper->getClassMapping('row-fluid'); ?>" id="advsearchformdiv">
			<div class="<?php echo $bootstrapHelper->getClassMapping('span12'); ?>">
			<?php
	...
	static function propertyDetails($option,$row,$configs,$owner){
		...
		//end featured
		$row->ncomments = count($row->comments);
		ob_start();
        ...
        else
        {
            ...
            $gscript .= '
                $$("#agooglemap").addEvent("click", function(e){
                    ...
                    $$("#liaphoto").removeClass("active");
                    $$("#liagooglemap").removeClass("active");
                    $$("#liastreetview").removeClass("active");
                    $$("#liavideo").removeClass("active");
                    $$("#liagooglemap").addClass("active");

					$$("#tabphoto").removeClass("active");
                    $$("#tabgoogle").removeClass("active");
					$$("#tabstreet").removeClass("active");
                    $$("#tabvideo").removeClass("active");
					$$("#tabgoogle").addClass("active");
                });
                    
                $$("#aphoto").addEvent("click", function(e){
                    $$("#liaphoto").removeClass("active");
                    $$("#liagooglemap").removeClass("active");
                    $$("#liastreetview").removeClass("active");
                    $$("#liavideo").removeClass("active");
                    $$("#liaphoto").addClass("active");
                ...
                $$("#avideo").addEvent("click", function(e){
                    $$("#liaphoto").removeClass("active");
                    $$("#liagooglemap").removeClass("active");
                    $$("#liastreetview").removeClass("active");
                    $$("#liavideo").removeClass("active");
                    $$("#liavideo").addClass("active");
                ...
                $$("#astreetview").addEvent("click", function(e){
                    ...
                    $$("#liastreetview").removeClass("active");
                    $$("#liavideo").removeClass("active");
                    $$("#liastreetview").addClass("active");

					$$("#tabphoto").removeClass("active");
                    $$("#tabgoogle").removeClass("active");
					$$("#tabstreet").removeClass("active");
                    $$("#tabvideo").removeClass("active");
					$$("#tabstreet").addClass("active");
                });   
		...
		if((($themename == "default") || ($themeobj->default_duplicate == 1)) && ($row->lat_add != "" && $row->long_add != "")){
		...
		if($row->show_address == 1){
			...
			if(OSPHelper::getLanguageFieldValue($row,'price_text') != ""){
				$addInfo1[] = OSPHelper::getLanguageFieldValue($row,'price_text');
			}elseif(($row->price > 0) and ($row->price_call == 0)){
		...
		ob_start();
		if(OSPHelper::getLanguageFieldValue($row,'price_text') != ""){
			echo " <span class='pricetext'>".OSPHelper::getLanguageFieldValue($row,'price_text')."</span>";
		}elseif($row->price_call == 0){
			if($row->price > 0){
				?>
	 ...
	 * @param unknown_type $coupon
	 */
	function updateFormStep1WithMembership($option,$rows){
		global $bootstrapHelper, $mainframe,$jinput,$configClass;
		$db = JFactory::getDBO();
        if(HelperOspropertyCommon::isAgent()){
            $usertype = 0;
