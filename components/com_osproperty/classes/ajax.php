...
class OspropertyAjax
{
    ...
    static function display($option, $task)
    {
        ...
        switch ($task) {
            ...
            case "ajax_loadstatecity":
                ...
                OspropertyAjax::loadStateCity($option, $country_name, $country_id, $state_id, $city_id, $state_name,'input-large');
                break;
            case "ajax_loadstatecityBackend":
                ...
                $state_id = $jinput->getInt('state_id', '0');
                $city_id = $jinput->getInt('city_id');
                OspropertyAjax::loadStateCityBackend($option, $country_name, $country_id, $state_id, $city_id, $state_name,'input-large');
                break;
            ...
            case "ajax_loadcityAddProperty":
                ...
                $state_id = $jinput->getInt('state_id', '0');
                $city_id = $jinput->getInt('city_id');
                OspropertyAjax::loadCity($option, $state_id, $city_id, $state_name, 0, 'input-large');
                break;
            ...
            switch($theme){
				...
				case "theme2":
                    switch($layout) {
                        case "details":
                            ?>
                            <a title="<?php echo JText::_('OS_ADD_TO_COMPARE_LIST');?>" onclick="javascript:osConfirmExtend('<?php echo $msg; ?>','ajax_addCompare','<?php echo $id ?>','<?php echo JURI::root() ?>','compare<?php echo $row->id;?>','theme2','details')" href="javascript:void(0)" class="compareLink">
            ...
            switch($theme){
				...
				case "theme2":
                    switch($layout) {
                        case "details":
                            ?>
                            <a title="<?php echo JText::_('OS_REMOVE_FROM_COMPARE_LIST');?>" onclick="javascript:osConfirmExtend('<?php echo $msg; ?>','ajax_removeCompare','<?php echo $id ?>','<?php echo JURI::root() ?>','compare<?php echo $id;?>','theme2','details')" href="javascript:void(0)" class="compareLinkActive">
                                <span class="edicon edicon-calculator"></span>
                            </a>
                            <?php
                            break;
     ...
     * @param unknown_type $option
     */
    static function loadStateCityBackend($option, $country_name, $country_id, $state_id, $city_id, $state_name, $class='input-medium')
    {
        global $jinput, $mainframe;
        @header('Content-Type: text/html; charset=utf-8');
        $db = JFactory::getDBO();
     ...
     * @param unknown_type $option
     */
    static function loadStateCity($option, $country_name, $country_id, $state_id, $city_id, $state_name , $class='input-medium')
    {
        global $jinput, $mainframe;
        @header('Content-Type: text/html; charset=utf-8');
        $db = JFactory::getDBO();
	...
	public static function checkingVersion(){
		...
		//checking to see whether if "OS Property installer" plugin is installed and published or not. 
		$db = JFactory::getDbo();
		$db->setQuery("Select count(extension_id) from #__extensions where `element` = 'osproperty' and `folder` = 'installer'");
		$count = $db->loadResult();
		if($count == 0){
			$result = array();
			$result['status'] = 0;
			...
			{
				$url = 'https://www.joomdonation.com/images/osproperty/version.txt';
				$ch = curl_init();
				...
				if ($latestVersion)
				{
					...
					<img src="<?php echo JUri::root();?>media/com_osproperty/assets/images/noupdated.png" />
					<?php
					echo '<span class="version-checking-alert">Click <a href="https://www.joomdonation.com/joomla-extensions/os-property-joomla-real-estate.html">here</a> to get update</span>';
				}
			}
