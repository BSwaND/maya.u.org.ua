...
class modOSpropertyramdomHelper extends JObject
{
	...
	function getProperty()
	{
		...
		if($propertyids != "")
		{
			...
			$propertyids    = \Joomla\Utilities\ArrayHelper::toInteger($propertyids);
		}
		else
		{
			$propertyids    = array();
        ...
        if($exclude_property_ids != "")
        {
            ...
            $exclude_property_ids       = \Joomla\Utilities\ArrayHelper::toInteger($exclude_property_ids);
        }
        else
        {
		...
		$where = " WHERE p.published = '1' AND p.approved = '1' ".$where_sql
			  .= ' and p.access IN (' . implode(',', JFactory::getUser()->getAuthorisedViewLevels()) . ')';
